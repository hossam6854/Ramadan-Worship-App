...
const BookmarksList = () => {
  const { bookmarks, removeBookmark, setSelectedSurah, setShowBookmarks, darkMode } = useQuran();
      ...
      {bookmarks.length === 0 ? (
        <div className="flex-1 flex items-center justify-center">
          <p className=" text-center">
            لم تقم بإضافة أي إشارات مرجعية بعد
          </p>
        </div>
      ) : (
        <div className="space-y-4 flex-1 overflow-y-auto cursor-pointer">
          {bookmarks.map((bookmark) => (
            <div
              key={bookmark.id}
              className={`flex items-center justify-between p-4  rounded-lg hover:bg-gray-100 cursor-pointer dark:hover:bg-gray-600 transition-colors ${
                darkMode ? "bg-gray-50 dark:bg-gray-700" : "bg-white text-gray-900"
              }`}
            >
              <button
